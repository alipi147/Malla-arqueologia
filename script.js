// Data extraída de tu documento "Pre-requisitos malla 2024.docx"
// "reqMode: 'OR'" se usa cuando el documento dice "o" (uno u otro)
// "req: 'TODO'" se usa para el final de la carrera
...
const mallaData = [
    ...
    { id: "s3_4", name: "Etnoarqueología", sem: 3, credits: 8, req: ["s2_1", "s2_3"] }, // General II y Etnografías
    { id: "s3_5", name: "Estadística I", sem: 3, credits: 7, req: ["s2_5"] }, // Matemática
    { id: "s3_6", name: "Teoría Arqueológica I", sem: 3, credits: 7, req: ["s2_1", "s1_3"] }, // General II y Epistemología
    { id: "s3_7", name: "Optativo Formación General", sem: 3, credits: 3, req: [] },
    { id: "s3_8", name: "Optativo Formación Teológica", sem: 3, credits: 5, req: [] },
    ...
    { id: "s4_3", name: "Arqueología del Norte Árido de Chile", sem: 4, credits: 7, req: ["s2_1", "s3_3"] }, // Gen II y América II
    ...
    { id: "s4_7", name: "Optativo Formación General", sem: 4, credits: 5, req: [] },
    { id: "s4_8", name: "Optativo Formación Teológica", sem: 4, credits: 3, req: [] },

    // SEMESTRE 5
    { id: "s5_1", name: "Laboratorio de Arte Rupestre", sem: 5, credits: 8, req: ["s2_1"] },
    { id: "s5_2", name: "Arqueología de Chile Semiárido", sem: 5, credits: 7, req: ["s2_1", "s2_2"] }, // Gen II y América I
    { id: "s5_3", name: "Arqueología de Chile Centro-Sur", sem: 5, credits: 7, req: ["s2_1", "s2_2"] }, // Gen II y América I
    ...
    { id: "s5_6", name: "Formación Complementaria", sem: 5, credits: 10, req: [] },

    // SEMESTRE 6
    { id: "s6_1", name: "Laboratorio de Especialización I", sem: 6, credits: 10, req: ["s3_1", "s3_2"], reqMode: "OR" }, // Lítica O Botánica
    { id: "s6_2", name: "Laboratorio de Bioarqueología", sem: 6, credits: 8, req: ["s2_1", "s5_5"] }, // Gen II y Ética
    { id: "s6_3", name: "Arqueología Histórica", sem: 6, credits: 8, req: ["s2_1", "s1_5"] }, // Gen II y Intro Cs Sociales
    { id: "s6_4", name: "Arqueología de la Patagonia Chilena", sem: 6, credits: 7, req: ["s2_2"] }, // América I
    { id: "s6_5", name: "Gestión del Patrimonio", sem: 6, credits: 8, req: ["s2_1", "s5_5"] }, // Gen II y Ética
    ...
    { id: "s7_2", name: "Arqueología Contemporánea", sem: 7, credits: 7, req: ["s6_3", "s4_6"] }, // Histórica y Teoría II
    ...
    { id: "s7_6", name: "Formación Complementaria", sem: 7, credits: 10, req: [] },
    ...
    { id: "s8_5", name: "Teoría Arqueológica V", sem: 8, credits: 8, req: ["s6_6", "s7_5"] }, // Teoría III y IV

    // SEMESTRE 9
    { id: "s9_1", name: "Seminario de Grado", sem: 9, credits: 15, req: ["TODO"] },
    { id: "s9_2", name: "Práctica Profesional", sem: 9, credits: 25, req: ["TODO"] },

    // SEMESTRE 10
    { id: "s10_1", name: "Seminario de Título", sem: 10, credits: 50, req: ["TODO"] }
];

// Estado inicial (cargar de memoria local si existe)
...
function renderMalla() {
    ...
    for (let i = 1; i <= 10; i++) {
        ...
        ramosSemestre.forEach(ramo => {
            ...
            card.className = 'ramo-card locked'; // Por defecto bloqueado
            card.onclick = () => toggleRamo(ramo.id);
            
            card.innerHTML = `
                <span>${ramo.name}</span>
                <span class="creditos">${ramo.credits} cred.</span>
            `;
...
function checkPrerequisites(ramo) {
    // Caso especial: Todo (Seminarios y Práctica)
    // Asumimos que "Todo" requiere tener TODOS los ramos hasta el semestre 8 aprobados
    if (ramo.req.includes("TODO")) {
        const ramosAnteriores = mallaData.filter(r => r.sem <= 8);
        return ramosAnteriores.every(r => progress[r.id] === true);
    }

    if (ramo.req.length === 0) return true; // Sin prerrequisitos

    // Lógica OR (uno u otro)
    if (ramo.reqMode === 'OR') {
        return ramo.req.some(reqId => progress[reqId] === true);
    }

    // Lógica AND (todos los prerrequisitos, por defecto)
...
function toggleRamo(id) {
    const ramo = mallaData.find(r => r.id === id);
    const card = document.getElementById(`card-${id}`);

    // Solo permitir toggle si está aprobado o si está disponible
    if (card.classList.contains('locked') && !card.classList.contains('approved')) {
        return; // No hacer nada si está bloqueado
    }

    if (progress[id]) {
        delete progress[id]; // Desaprobar
    } else {
        progress[id] = true; // Aprobar
    }

    // Guardar y actualizar visualmente
