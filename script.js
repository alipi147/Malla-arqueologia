// Datos actualizados (misma data que antes)
const mallaData = [
    // --- SEMESTRE 1 ---
...
function renderMalla() {
    ...
    for (let i = 1; i <= maxSem; i++) {
        ...
        // 1. Título del Semestre
        const title = document.createElement('div');
        title.className = 'semestre-title';
        title.textContent = `Sem ${i}`;
        col.appendChild(title);

        // 2. NUEVO: Calcular créditos totales del semestre
        const ramosSemestre = mallaData.filter(r => r.sem === i);
        const totalCreditos = ramosSemestre.reduce((sum, r) => sum + r.credits, 0);

        // 3. NUEVO: Crear el elemento visual de los créditos
        const creditsDiv = document.createElement('div');
        creditsDiv.className = 'semestre-credits';
        creditsDiv.id = `credits-sem-${i}`; // ID único para actualizarlo luego
        creditsDiv.textContent = `0 / ${totalCreditos}`; // Valor inicial
...
function updateStatus() {
    // 1. Actualizar colores y bloqueos de las tarjetas
    ...
    // 2. NUEVO: Actualizar contadores de créditos por semestre
    const maxSem = Math.max(...mallaData.map(r => r.sem));
    for (let i = 1; i <= maxSem; i++) {
        const ramosSem = mallaData.filter(r => r.sem === i);
        
        // Sumar créditos de ramos aprobados en este semestre
        const creditosAprobados = ramosSem
            .filter(r => progress[r.id] === true)
            .reduce((sum, r) => sum + r.credits, 0);
        
        // Calcular total del semestre (esto es fijo, pero lo recalculamos para tener el dato)
...
function toggleRamo(id) {
    ...
    updateStatus(); // Esto disparará la actualización de créditos
}
